...
function migrateLegacyStructure(doc, userId) {
  ...
  const migrated = { ...doc };
  const preserveFields = ["availableAt", "assignedAt", "completedAt", "quizPassedAt", "microActionCompleted", "survey", "actions", "module", "topic", "title", "description", "details", "level", "metadata"];
...
module.exports = {
  ...
  markQuizAttempt: async ({ userId, learningId, quizId, update }) => {
      ...
      const entry = getLearningEntry(doc, learningId);
      if (entry) {
          // Find if there's an open (pending) attempt for this quiz
          let attempt = entry.attempts.find(a => a.quizId === quizId && a.status !== "completed");
