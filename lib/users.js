...
export async function upsertUserProfile(partialProfile = {}) {
  ...
  const updated = {
    ...
    lastActivityAt: partialProfile.lastActivityAt ?? existing.lastActivityAt ?? now,
    lastUsageLogAt: partialProfile.lastUsageLogAt ?? existing.lastUsageLogAt ?? null,
    notifiedStreak20h: partialProfile.notifiedStreak20h ?? existing.notifiedStreak20h ?? false,
    notifiedUsage4h: partialProfile.notifiedUsage4h ?? existing.notifiedUsage4h ?? false,
    conversationReference: partialProfile.conversationReference ?? existing.conversationReference ?? null,
    id: existing.id,
