...
export async function syncLearningAssignment(userId, forceResync = false) {
  ...
  const availableLearnings = progress.learnings
    ...
    .sort((a, b) => {
      ...
      return (moduleA?.order || Infinity) - (moduleB?.order || Infinity);
    });
  ...
  // 2. Fallback to existing logic if no explicitly available module is found
  if (!targetModule) {
  ...
  if (!entry) {
    // This case should ideally be covered by the first block or existing logic.
    // If targetModule is found but entry is null, it means the module
    // exists in catalog but not in user's progress - create it.
    entry = {
      learningId: targetModule.id,
      status: "available",
      attempts: [],
      createdAt: nowIso(),
      updatedAt: nowIso(),
      availableAt: nowIso(), // Set available now if it's the first time
    };
    progress.learnings.push(entry);
    await saveResponseProgress(progress); // Save the new entry
  }
