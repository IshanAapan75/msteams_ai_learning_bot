  ...
  constructor(conversationState) {
    ...
    this.onMessage(async (context, next) => {
      ...
      if (text === "start quiz") {
        ...
        console.log(`[Bot] Answering question ${state.questionIndex + 1}/${state.currentQuiz.questions.length} of Quiz ${state.currentQuizIndex + 1}/${state.allQuizzes.length}`);
        ...
        // Check if current quiz has more questions
        if (state.questionIndex < state.currentQuiz.questions.length) {
          console.log(`[Bot] More questions in current quiz, sending next question`);
          await this.sendQuestion(context, state);
        } else {
          // Current quiz finished
          await context.sendActivity(
            `📊 Quiz "${state.currentQuiz.title}" completed!\nScore: ${state.currentQuizScore}/${state.currentQuiz.questions.length}\n\n`
          );
  ...
  async moveToNextQuiz(context, state) {
    ...
    if (state.currentQuizIndex < state.allQuizzes.length) {
      ...
      await context.sendActivity(
        ...
        `Total Score: ${state.totalScore}/${totalQuestions}\n` +
        `Percentage: ${Math.round((state.totalScore / totalQuestions) * 100)}%\n\n` +
        `Great job! 🎊`
      );
